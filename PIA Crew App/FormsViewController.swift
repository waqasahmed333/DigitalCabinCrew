...
class FormsViewController: UIViewController, UISearchResultsUpdating{
        ...
        DocumentServices.getDocuments(section.trimmingCharacters(in: CharacterSet.whitespaces)) { (documentSectionArray, error) in
            ...
            if error == nil {
                ...
                if !documentSectionArray!.isEmpty {
                    ...
                    alert("Message", Body: "Forms Data not available")
                    Indicator.stop()
...
extension FormsViewController : UITableViewDelegate , UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "detailsCell") as! FormsDocumentTableViewCell
        ...
        cell.delete.addTarget(self, action: #selector(FormsViewController.DeleteDocumentUsingFileManager(_:)), for: .touchUpInside)
        ...
        if self.documents[(indexPath as NSIndexPath).row].name.isFileAvailable() {
            ...
            cell.download.addTarget(self, action: #selector(FormsViewController.open(_:)), for: .touchUpInside)
            ...
            cell.download.addTarget(self, action: #selector(FormsViewController.download(_:)), for: .touchUpInside)
            cell.download.tag = (indexPath as NSIndexPath).row
    ...
    @objc func download(_ sender:UIButton) {
        ...
        let cell = self.tableViewDetails.cellForRow(at: IndexPath(row: sender.tag, section: 0)) as! FormsDocumentTableViewCell
        ...
        alertC.addAction(UIAlertAction(title: "Confirm", style: UIAlertAction.Style.destructive, handler: { action in
            ...
            DownloadServices.downloadMultipleFiles(urls) { (index, percentage, error) in
                ...
                if error == nil {
                    ...
                    let cell : FormsDocumentTableViewCell? = self.tableViewDetails.cellForRow(at: IndexPath(row: index, section: 0)) as? FormsDocumentTableViewCell
