...
class ManualsViewController: UIViewController, UISearchResultsUpdating{
...
///    var progressView:MRProgressOverlayView!
    ...
    override func viewDidLoad() {
        ...
        DocumentServices.getDocuments(ac.trimmingCharacters(in: CharacterSet.whitespaces)) { (documentSectionArray, error) in
...
extension ManualsViewController : UITableViewDelegate , UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.delete.addTarget(self, action: #selector(ManualsViewController.DeleteDocumentUsingFileManager(_:)), for: .touchUpInside)
        ...
        if self.documents[(indexPath as NSIndexPath).row].name.isFileAvailable() {
            ...
            cell.download.addTarget(self, action: #selector(ManualsViewController.open(_:)), for: .touchUpInside)
            ...
            cell.download.addTarget(self, action: #selector(ManualsViewController.download(_:)), for: .touchUpInside)
            cell.download.tag = (indexPath as NSIndexPath).row
...
//                print("Could not remove file: \(error)")
            ...
            var downloadURL = self.documents[sender.tag].url!
            
            if (NetworkUtils.getActiveServerURL().contains("crew2")) {
                downloadURL = downloadURL.replacingOccurrences(of: "crewserver1.piac.com.pk/", with: "piacconnect.piac.com.pk/FO/")
            }

            
            DownloadServices.downloadFile(downloadURL) { (percentage, error) in
