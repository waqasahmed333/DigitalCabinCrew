...
struct SectionField  {
    ...
    var startLocationInSection:Int
    var length:Int
    var regex:String
...
class FPPlan: Object {
    ...
    @objc dynamic var contentHashcode = ""
    @objc dynamic var xml_form_file = ""
    @objc dynamic var createdOn = Date()
    @objc dynamic var downloadedOn : Date?
    @objc dynamic var signedOn : Date?
    @objc dynamic var uploadedOn : Date?
    
    
    
}

class FPFlight: Object {
    @objc dynamic var aircraft = ""
    @objc dynamic var arrStation = ""
    @objc dynamic var crewId = ""
    @objc dynamic var flightDate = Date()
    @objc dynamic var depStation = ""
    @objc dynamic var documentName = ""
    @objc dynamic var flightNumber = ""
    @objc dynamic var sta = Date()
    @objc dynamic var std = Date()
    @objc dynamic var url = ""
    @objc dynamic var createdOn = Date()

    let plans = List<FPPlan>()
}

class FPFlightListViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, URLSessionDelegate {
    ...
    let webRequestName = WebRequest.FP_FLIGHT_LIST;
    
    let deleteObjectAfterDays = 5
    
    let minDelaySeconds = 1 * 60
    
    let standardDelaySeconds = 15 * 60
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if ( RealmUtils.shouldCallWebRequest(key: WebRequest.FP_FLIGHT_LIST, delayType: WebRequest.DELAY_TYPE_STANDARD) ) {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
       ...
       cell.sta.text = StringUtils.getHHMMFromDateTime(flightPlanList[(indexPath as NSIndexPath).row].sta)
        ...
        if flightPlanList[(indexPath as NSIndexPath).row].documentName.isFileAvailable() {
            ...
            cell.download.addTarget(self, action: #selector(FPFlightListViewController.open(_:)), for: .touchUpInside)
            ...
            cell.download.addTarget(self, action: #selector(FPFlightListViewController.download(_:)), for: .touchUpInside)
            cell.download.tag = (indexPath as NSIndexPath).row
            ...
            DownloadServices.downloadFlightPlan2(self.flightPlanList[sender.tag].url) { (percentage, error) in
                ...
                if error == nil {
                    ...
                    if percentage == 1 {
                        ...
                        cell.btnFill.addTarget(self, action: #selector(FPFlightListViewController.open(_:)), for: .touchUpInside)
